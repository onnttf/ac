...
func GenerateCode(ctx context.Context) (string, error) {
	...
	for i := 0; i < maxAttempts; i++ {
		// Generate a new code
		tmpCode := code.GenerateUserCode()

		// Check if the code exists (including deleted records)
		record, err := dal.NewRepo[model.User]().Query(ctx, database.DB, func(db *gorm.DB) *gorm.DB {
			return db.Unscoped().Where(model.User{Code: tmpCode})
		})
		if err != nil {
...
func Validate(ctx context.Context, systemCode, code string) (bool, error) {
	...
	record, err := dal.NewRepo[model.User]().Query(ctx, database.DB, func(db *gorm.DB) *gorm.DB {
		return db.Where(model.User{SystemCode: systemCode, Code: code})
	})
	if err != nil {
